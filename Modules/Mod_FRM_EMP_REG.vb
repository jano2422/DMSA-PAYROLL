...
"   (HR_EMPLOYEE_CLIENT_TRANSFER_DTL AS D " &
"    INNER JOIN " &
"       (SELECT T1.EMPLOYEE_ID, MAX(T1.ID) AS MaxID " &
"        FROM HR_EMPLOYEE_CLIENT_TRANSFER_DTL AS T1 " &
"        INNER JOIN " &
"           (SELECT EMPLOYEE_ID, MAX(DATE_STARTED) AS MaxDateStarted " &
"            FROM HR_EMPLOYEE_CLIENT_TRANSFER_DTL " &
"            GROUP BY EMPLOYEE_ID) AS T2 " &
"        ON T1.EMPLOYEE_ID = T2.EMPLOYEE_ID " &
"       AND T1.DATE_STARTED = T2.MaxDateStarted " &
"        GROUP BY T1.EMPLOYEE_ID) AS X " &
"    ON D.EMPLOYEE_ID = X.EMPLOYEE_ID AND D.ID = X.MaxID) " &
"ON B.EMPLOYEE_ID = D.EMPLOYEE_ID) " &
"LEFT JOIN TBL_CLIENT_SUB AS E ON D.SUB_CLIENT_ID = E.SUB_CLIENT_ID "
